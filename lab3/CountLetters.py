...
def various_counts(*tmpStr):
      tmpList = tmpStr
      hashmap = {}
      tmpSentence = "".join(tmpList)

      for i in tmpSentence:
            if i.isalpha():
                  if i in hashmap:
                        hashmap[i] += 1
                  else:
...
def CountLetters():
      if len(sys.argv) == 2:
            freq = letter_count(sys.argv[1])
      elif len(sys.argv) == 3:
            freq = double_count(sys.argv[1], sys.argv[2])
      else:
            freq = various_counts(*sys.argv[1:])
      sorted_freq = sorted(freq, reverse=True)
      for item in sorted_freq:
            print ('%s:%d' % (item, freq[item]), end=' ')
