...
Task Description:
    ...
    print("The difference is:%d The summation is:%d The summation of all input is:%d The number of even numbers is:%d The values in the list are: %s" % (myMath.subtraction(maxNum, minNum), myMath.add(maxNum, minNum), myMath.sumTotal(intList), myMath.evenNum(intList), intList))
...
def myMain():
    if len(sys.argv) != 2:
        print("Please input the numbers in one string!")
        return
    strNums = sys.argv[1]
    strList = strNums.split(',')
    intList = [int(i) for i in strList]
    maxNum = myMath.maximum(intList)
    minNum = myMath.minimum(intList)
    difference = myMath.subtraction(maxNum, minNum)
    summation = myMath.add(maxNum, minNum)
    totalSum = myMath.sumTotal(intList)
    evenCount = myMath.evenNum(intList)
    if minNum < 5:
        intList = myMath.clear(intList)
    print("The difference is:%d The summation is:%d The summation of all input is:%d The number of even numbers is:%d The values in the list are: %s" % (difference, summation, totalSum, evenCount, intList))
